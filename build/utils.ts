...
export const create = (path: string): Promise<void> =>
  new Promise((resolve, reject) =>
    exists(path).then((exists) =>
      !exists ? mkdir(path, (err) => (err ? reject(err) : resolve())) : resolve(),
    ),
  )

export const run = (cmd: string, cwd?: string): Promise<string> =>
  new Promise((resolve, reject) =>
    exec(cmd, { cwd }, (err, out) => (err ? reject(err) : resolve(out))),
  )

export const files = (
  path: string,
  filter: (file: string) => boolean = (_) => true,
): Promise<string[]> =>
  new Promise((resolve, reject) =>
    readdir(path, (err, files) => (err ? reject(err) : resolve(files.filter(filter)))),
  )

export const copyJson = (
  src: string,
  dst: string,
  rewriteFields?: { [key: string]: any },
): Promise<void> =>
  new Promise((resolve, reject) =>
